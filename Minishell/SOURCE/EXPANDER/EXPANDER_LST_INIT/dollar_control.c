...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/10 11:32:31 by tamadan           #+#    #+#             */
/*   Updated: 2024/01/10 11:32:32 by tamadan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*dollar_control(char *str, int *end, t_prime *g_prime, int *env)
{
	int		i;
	int		start;
	char	*substr;
	...
	else if (put_syntxerror(str[i]))
		synerr_print(g_prime, "Syntax error", 1);
	else if (spcl_chars(str[i]))
	...
	if (ret_null(str[i]) || spcl_chars(str[i]))
		i++;
	*end = i;
	return (substr);
}
